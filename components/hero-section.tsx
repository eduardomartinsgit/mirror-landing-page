...
export function HeroSection() {
  ...
  const typingTexts = ta<string>("hero.typingTexts")
  const pills = ta<{ label: string }>("hero.pills")
  ...
  const pillIcons = [Brain, Shield, Sparkles]
  const pillColors = ["#E91E8C", "#00D4FF", "#7B2FBF"]
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
          ...
          <ScrollReveal animation="fade-right" className="flex-1 text-center lg:text-left">
            ...
            <p className="text-base sm:text-lg text-muted-foreground max-w-md mb-8 leading-relaxed mx-auto lg:mx-0 text-center lg:text-left">
              ...
              <span className="text-[#E91E8C]">{t("hero.subtitleHighlight")}</span>{t("hero.subtitleAfter")}
            </p>
            ...
            <div className="grid grid-cols-3 gap-3 mb-10 max-w-md mx-auto lg:mx-0">
              {pills.map((pill, i) => {
                ...
                const color = pillColors[i]
                return (
                  <div key={i} className="group flex flex-col items-center gap-2 py-4 px-3 rounded-2xl bg-card/60 backdrop-blur-sm border border-border/50 hover:border-white/20 transition-all duration-300 text-center">
                    ...
                    <p className="text-xs font-semibold text-foreground leading-tight">{pill.label}</p>
                  </div>
                )
              })}
            </div>
